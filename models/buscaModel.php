...
class BuscaModel {
	...
	public static function mdlBuscaPropiedadesAjax($Tabla, $Tipo) {
		...
		if ($Tipo == "") {
			$Statement = Conexion::conectar()->prepare("SELECT * FROM $Tabla ORDER BY fechaRegistro DESC");
			$Statement->execute();
			return $Statement->fetchAll();
		} else {
			$Statement = Conexion::conectar()->prepare("SELECT * FROM $Tabla WHERE categoria = :categoria ORDER BY fechaRegistro DESC");
	...
	public static function mdlBuscaPropiedades($Tabla, $Tipo) {
		...
		if ($Tipo == "") {
			$Statement = Conexion::conectar()->prepare("SELECT id, status, categoria, fotoPrincipal, areaTerreno, habitaciones, banos, titulo, precio, direccion FROM $Tabla ORDER BY fechaRegistro DESC");
			$Statement->execute();
			return $Statement->fetchAll();
		} else {
			$Statement = Conexion::conectar()->prepare("SELECT id, status, categoria, fotoPrincipal, areaTerreno, habitaciones, banos, titulo, precio, direccion FROM $Tabla WHERE categoria = :categoria ORDER BY fechaRegistro DESC");
			$Statement->bindParam(":categoria", $Tipo, PDO::PARAM_STR);
			$Statement->execute();
			return $Statement->fetchAll();
		}
	...
	public static function mdlBuscaPropiedadesrecientes($tabla) {
		$Statement = Conexion::conectar()->prepare("SELECT  id,status,moneda,categoria, fotoPrincipal, areaTerreno, habitaciones, banos, titulo, precio, direccion FROM $tabla WHERE oferta=0 ORDER BY fechaRegistro DESC , destacada DESC LIMIT 9");
		$Statement->execute();
		return $Statement->fetchAll();
	}

	public static function mdlHayOfertas($tabla) {
		$Statement = Conexion::conectar()->prepare("SELECT id FROM $tabla WHERE oferta = 1 ORDER BY fechaRegistro DESC");
		$Statement->execute();
	...
	public static function mdlBuscaPropiedadesOferta($tabla) {
		$Statement = Conexion::conectar()->prepare("SELECT  id,status,categoria, fotoPrincipal,moneda, areaTerreno, habitaciones, banos, titulo, precio, direccion, precioOferta FROM $tabla WHERE oferta = 1 ORDER BY fechaRegistro DESC LIMIT 9");
		$Statement->execute();
		return $Statement->fetchAll();
	}
}
